...
@Component({
  ...
  styleUrl: './fixed-price.component.css'
})
export class FixedPriceComponent {
  ...
  ngOnInit(): void {
    this.httpService.getFixedPriceReviews(this.providerID)
      .subscribe((resp: any) => {
        this.reviews = resp
        this.loading = false
      })
  ...
  writeResponse(reviewId: number, modal: any) {
    ...
    modalRef.result.then((result: any) => {
      ...
      this.httpService.respondToFixedPriceReview(this.providerID, request)
        .subscribe((resp: any) => {
          if(resp.responded){
            this.ngOnInit()
          }
